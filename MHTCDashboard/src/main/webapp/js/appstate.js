...
var AS = (function($) {
	function AppState() {
		...
		/********Array where multiple metrics selected are stored*********/
		this.selected_multiple_years = [];
		/********Array where multiple years selected are stored*********/
		var array_years_global = [];
		this.metric_category_table = new Object();
		/********hashtable to suggest duplicate metric categories*********/
	...
	AppState.prototype.loadFunction = function() {
		...
		$('#map').usmap(
				{
					...
					'mouseover' : function(event, data) {
						$("#mapTitle").text(
								States.getStateByAbbreviation(data.name).name
		...
		nationalList = document.getElementById(id2).getElementsByClassName(
				"submenu");
		this.addInMetricTable(nationalList, id2);

		//console.log(this.metric_category_table[keyofTable]);
		var id3 = "talentProfileList"
		talentList = document.getElementById(id3).getElementsByClassName(
				"submenu");
		this.addInMetricTable(talentList, id3);

		var id4 = "costProfileList"
		costList = document.getElementById(id4).getElementsByClassName(
				"submenu");
		this.addInMetricTable(costList, id4);

		var id5 = "economyProfileList"
		economyList = document.getElementById(id5).getElementsByClassName(
				"submenu");
	...
	AppState.prototype.getDuplicateMetricCategory = function(currentParentId,
			id) {
		var mlist = [];
	...
	AppState.prototype.SetHeatMap = function(ind, baseColor, tag_id, year_in) {
		...
		var query = DQ.create().addState(selected_states_global).addMetric(
				Metrics.getMetricByID(this.currentind).getName());
		query
				.execute(function(multiData) {
					setTimeout(function() {
						/*******************************************************************************/
						...
						array_years.sort(function(a, b) {
							return b - a
						});
						array_years_global.sort(function(a, b) {
							return a - b
						});
						...
						for (var k = array_years.length - 1; k >= 0; k--) {
							if (year_in != array_years[k]) {
								liststring += '<li ><button  class="tableTimeLineButton" onClick="heatmapButtonClicked('
										+ array_years[k]
										+ ')" id="clicktable'
										+ array_years[k]
										+ '">'
										+ array_years[k] + '</button></li>';
							} else {
								liststring += '<li id="clicktable' + year_in
										+ '"><button  class="active" >'
										+ year_in + '</button></li>';
							}
						}
						...
						var sentinel;
						for (var i = 0; i < multiData.length; i++) {
							if ((multiData[i][0].state.abbr != 'US')
									&& (multiData[i][0].state.abbr != 'DC')) {
								j = 0;
								sentinel = false;
								while ((!sentinel)
										&& (j < multiData[i][0].dataPoints.length)) {
								...
								if (sentinel) {
									as.ordered_states_metrics[i] = {
										value_element : multiData[i][0].dataPoints[j].value,
								...
								.usmap(
										{
											...
											'mouseover' : function(event, data) {
												var found = $
														.map(
																as.ordered_states_metrics,
																function(val, i) {
																	var indexcounter;
																	var percentage = "";
																	var return_value_element;
																	if (val.state_element == States
																			.getStateByAbbreviation(data.name).abbr) {
																		if (as.ordered_states_metrics[0].binName_element != 'National')
																			indexcounter = i + 1;
																		else
																			indexcounter = as.ordered_states_metrics.length
																					- i;
																		if (val.value_element == 0.0) {
																			...
																			if (as.ordered_states_metrics[0].metricType == "percentage") {
																				...
																				return_value_element = Math
																						.round(return_value_element * 100)
																						/ 100
																						+ "%";
																			}
																				...
																				return_value_element = Math
																						.round(val.value_element * 100) / 100;
																			}
																			return "Ranking: "
																					+ indexcounter
																					+ " Value: "
																					+ return_value_element;
																		}
																	}

																});

												$("#graphStatesHeatMapPos0")
														.text(
																"State: "
																		+ States
																				.getStateByAbbreviation(data.name).name);
												$("#graphStatesHeatMapPos1")
														.text(found[0]);
											},
											click : function(event, data) {
												as.clickCallback(data.name);
						...
						for (var i = 0; i < as.ordered_states_metrics.length; i++) {
							if (multiData[0][0].metric.binName != "National") {
								newColor = getNewColor(
										baseColor,
										as.ordered_states_metrics[i].value_element,
										as.ordered_states_metrics[0].value_element);
							} else {
								newColor = getNewColor(
										baseColor,
										as.ordered_states_metrics[0].value_element
							...
							$(tag_id).usmap('changeStateColor',
									as.ordered_states_metrics[i].state_element,
									newColor);
						}
						...
						selheatmapmeter
								.append('<table border="1" cellpadding=0  cellspacing="0">');
						var r;
						var return_value_element;
						...
						if (multiData[0][0].metric.binName == "National") {
							...
							for (var i = 0; i < as.ordered_states_metrics.length; i++) {
								if (as.ordered_states_metrics[i].metricType == "percentage")
									return_value_element = Math
											.round((as.ordered_states_metrics[i].value_element * 100) * 100)
											/ 100 + "%";
								else
									return_value_element = as.ordered_states_metrics[i].value_element;
								if ((i == 0)
										|| (as.ordered_states_metrics[i].state_element == as.stateAbbr)) {
									selheatmapmeter
											.append('<td><strong>'
													+ as.ordered_states_metrics[i].state_element
													+ ':</strong> '
									...
									r = i + 1;
									if ((r == as.ordered_states_metrics.length)
											&& (as.ordered_states_metrics[i].state_element != as.stateAbbr)) {
										selheatmapmeter
												.append('<td><strong>'
														+ as.ordered_states_metrics[i].state_element
						...
						selverticalheatmapmeter
								.append('<table border="1" cellpadding=0  cellspacing="0"  style="height: 10px;">');
						var r;
						var return_value_element;
						var enter = false;
						var state_name;
						...
						if (multiData[0][0].metric.binName != "National") {
							...
							for (var i = 0; i < as.ordered_states_metrics.length; i++) {
								if ((i == as.ordered_states_metrics.length - 1)) {
									position = i + 1;
									selverticalheatmapmeter
											.append('<tr><td  valign="top" bgcolor="'
													+ arrayNewColor[i]
													+ '">&nbsp;&nbsp;</td><td valign="top" align="left" nowrap="true"><strong>'
													+ as.ordered_states_metrics[i].state_element
													+ '</strong>Pos: '
													+ position + '</td></tr>');
										...
										if (!enter) {
											...
											if (written_to_detail) {
												...
												selverticalheatmapmeter
														.append('<tr><td  valign="top" bgcolor="'
																+ arrayNewColor[i]
																+ '">&nbsp;&nbsp;</td><td valign="top" align="left"></td></tr>');
											}
										} else {
											enter = false;
										}
							...
							for (var i = as.ordered_states_metrics.length - 1; i >= 0; i--) {
								if ((i == 0)) {
									position = -1
											* (i - as.ordered_states_metrics.length);
									selverticalheatmapmeter
											.append('<tr><td  valign="top" bgcolor="'
													+ arrayNewColor[i]
													+ '">&nbsp;&nbsp;</td><td valign="top" align="left" nowrap="true"><strong>'
									...
									if ((i == as.ordered_states_metrics.length - 1)
										...
										if (i == as.ordered_states_metrics.length - 1)
											position = -1
													* (pivotcounter
															- as.ordered_states_metrics.length - 1);
										else
											position = -1
													* (i - as.ordered_states_metrics.length);
										written_to_detail = true;
	 ...
	 * 
	 */
	AppState.prototype.showHeatMapGraph = function(ind, style_in, tag_id,
			year_in) {
		this.current_tab_style = style_in;
		document.getElementById("graphTitleHeatMap").innerHTML = 'Heat map: '
				+ Metrics.getMetricByID(ind).getName();
		//document.getElementById("graphCatHeatMap").innerHTML='Category: '+Metrics.getMetricByID(ind).binName;
		var baseColor = colorToHex(getStyleRuleValue('background-color',
				style_in));
		var degradedColor = getNewColor(baseColor, 110, 100);
		//console.log("error degradedColor : "+degradedColor);
		var degradationColorWindow = "rgb(" + hexToRgb(degradedColor).r + ","
				+ hexToRgb(degradedColor).g + "," + hexToRgb(degradedColor).b
				+ ")";
		//this.changeColorModalWindow(tag_id.substr(1),degradationColorWindow);
		// console.log("error2 : "+ind);
		if (year_in == -1) {
			stopHeatMapAnimation();
			/**Users could left the animation running......**/
	 ...
	 */
	AppState.prototype.setHeatMapOnHover = function(ind, style_in, tag_id,
			year_in) {
	...
	AppState.prototype.UnSetHeatMap = function(style) {
		...
		if (global_timer != null)
			/**This is in case someone left the heatmap running**/
			stopHeatMapAnimation();
		...
		for (var i = 0; i < selected_states.length; i++) {
			...
			if ((selected_states[i] != 'US') && (selected_states[i] != 'DC')) {
				if (this.stateAbbr == selected_states[i]) {
					$("#map").usmap('changeStateColor', selected_states[i],
							this.stateSelectedColor);
				} else {
					if (States.getStateByAbbreviation(selected_states[i])
							.isPeerState()) {
	...
	AppState.prototype.checkIfAllStatesChecked = function() {
		...
		if (size_array_states_t < counter_unselected) {
			/***If all states are unselected then only the SELECTALL Button may appear***/
		...
		} else if (counter_unselected == 0) {
			/** If all states are selected then the SELECTALL button gets blocked and the UNSELECTALL gets unblocked**/
			...
			/** some states have been selected, therefore both buttons may appear**/
			$("#selectall").prop("disabled", false);
			$("#unselectall").prop("disabled", false);
			$("#selectallmultiplemetric").prop("disabled", false);
			$("#unselectallmultiplemetric").prop("disabled", false);
	 ...
	 */
	AppState.prototype.set_initializer = function(title_prefix_in,
			this_graph_in) {
	...
	AppState.prototype.showMultiGraphOnSelected = function() {
		this.set_initializer("Compare to Selected: ",
				this.showMultiGraphOnSelected);
	...
	AppState.prototype.showMultipleMetricsStatesYears = function(year_in) {
		this.set_initializer("Compare to Selected: ",
				this.showMultipleMetricsStatesYears);
		cm.showMultipleMetricsStatesYears(this.selected,
				this.selected_multiple_metrics, year_in);
	...
	AppState.prototype.showMultiGraphOnTopTen = function() {
		if (this.currentind == null || this.currentind == undefined)
			return;
		this.set_initializer("Compare Top Ten States: ",
				this.showMultiGraphOnTopTen);
		//this.currentind = ind;
		this.selected = $.parseJSON(
				$.ajax({
					url : "data/peers/top?metric="
							+ Metrics.getMetricByID(this.currentind).getName()
							+ "&year=0",
					dataType : 'text',
					async : false,
	...
	AppState.prototype.showMultiGraphOnBottomTen = function() {
		if (this.currentind == null || this.currentind == undefined)
			return;
		this.set_initializer("Compare Bottom Ten States: ",
				this.showMultiGraphOnBottomTen);
		//this.currentind = ind;
		this.selected = $.parseJSON(
				$.ajax({
					url : "data/peers/bottom?metric="
							+ Metrics.getMetricByID(this.currentind).getName()
							+ "&year=0",
	...
	AppState.prototype.SelectUnSelectAllTabs = function(type_in) {
		...
		for (var i = 0; i < selected_states.length; i++) {
			if (selected_states[i] != 'US') {
				indexState = as.selected.indexOf(selected_states[i]);
				if (((type_in == 1) || (type_in == 3)) && (indexState == -1)) {
					/**SELECT ALL*/
					this.clickCallback(selected_states[i]);
				} else if (((type_in == 2) || (type_in == 4))
						&& (indexState != -1)) {
					this.clickCallback(selected_states[i]);
				}
			}
	...
	AppState.prototype.loadState = function(stateData) {
		...
		$.get("" + this.stateAbbr + "/table", function(data) {
			$("#stateTitle").text(
					States.getStateByAbbreviation(as.stateAbbr).getName());
	...
	AppState.prototype.changeColorPeerStates = function(style) {
		...
		var divboxcolorpeerstates = document
				.getElementById('boxcolorpeerstates');
		...
		var newColorModalWindow = getStyleRuleValue('background-color',
				modalcontentidstyle);
	...
	AppState.prototype.SelectUnselectMultipleMetric = function(metric_id_in,
		...
		if (option_in == 1) {
			...
			this.showHeatMapGraphReloaded(
					this.selected_multiple_metrics[lastpos], '#mbodyHeatMap',
					-1); //CALL THE HEATMAP BACK AGAIN BUT WITH THIS METRIC
			...
			if (this.selected_multiple_metrics.length > 0) {
				...
				this.showHeatMapGraphReloaded(
						...
						'#mbodyHeatMap', -1); //CALL THE HEATMAP BACK AGAIN BUT WITH THIS METRIC
			...
			this.showHeatMapGraphReloaded(this.selected_multiple_metrics[i],
					'#mbodyHeatMap', -1); //CALL THE HEATMAP BACK AGAIN BUT WITH THIS METRIC
	...
	AppState.prototype.HeatMapMeter = function(type_in, category_in,
			ordered_states_metrics, arrayNewColor, stateAbbr) {
		if (type_in == 1) {
			$("#verticalheatmapmeter").removeClass("hidden");
			//$("#mapTitle").text("Heat map for "+year_in);
			var selverticalheatmapmeter = $("#verticalheatmapmeter");
					...
					if ((i == ordered_states_metrics.length - 1)) {
						...
						selverticalheatmapmeter
								.append('<tr><td  valign="top" bgcolor="'
										+ arrayNewColor[i]
										+ '">&nbsp;&nbsp;</td><td valign="top" align="left" nowrap="true"><strong>'
										+ ordered_states_metrics[i].state_element
										+ '</strong>Pos: ' + position
										+ '</td></tr>');
					} else {
						if ((i == 0)
								|| (ordered_states_metrics[i].state_element == stateAbbr)) {
					...
					if ((i == 0)) {
						...
						if ((i == ordered_states_metrics.length - 1)
							...
							if (i == ordered_states_metrics.length - 1)
								position = -1
										* (pivotcounter
												- ordered_states_metrics.length - 1);
							else
								position = -1
										* (i - ordered_states_metrics.length);
							written_to_detail = true;
						} else {
							if (!written_to_detail) {
								state_name = null;
	...
	AppState.prototype.showHeatMapGraphReloaded = function(ind, tag_id, year_in) {
		...
		if (this.selected_multiple_metrics.length == 0) {
			...
			this.SetHeatMap(this.selected_multiple_metrics[ind], baseColor,
					tag_id, year_in);
		}
	};
