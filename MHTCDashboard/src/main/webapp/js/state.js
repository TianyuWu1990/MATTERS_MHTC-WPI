 ...
 */
 var States = (function() {
		...
		/**
		 * Given a string that could be part of a name or abbreviation, returns states that matches either
		 */
		publicInterface.searchForStates = function(searchText) {
			return states.filter(function(each) {
				var regex = new RegExp(searchText);
				
				return regex.test(each.name) || regex.test(each.abbr);
				
			});
		};
		
		/**
		 * Given a string that could be part of a name or abbreviation, returns states that don't match
		 */
		publicInterface.filterStates = function(searchText) {
			return states.filter(function(each) {				
				var regex = new RegExp(searchText);
